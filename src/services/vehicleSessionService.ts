...
async function findCard(rawValue: string, siteId: string): Promise<ParkingCardRecord> {
  const normalizedCard = normalizeCardNumber(rawValue);
  const normalizedQr = normalizeQrValue(rawValue);
  const [byCard, byQr] = await Promise.all([
  ...
  const matches = new Map([...byCard.docs, ...byQr.docs].map(item => [item.id, item]));
  if (matches.size !== 1) throw new Error(matches.size ? 'พบบัตรซ้ำ กรุณาให้ผู้ดูแลตรวจสอบข้อมูล' : 'ไม่พบบัตรจอดรถในพื้นที่นี้');
