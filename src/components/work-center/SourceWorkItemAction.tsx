...
export default function SourceWorkItemAction({
  ...
  return (
    <div className={`flex flex-col gap-2 ${className}`}>
      ...
      {activeItem ? (
        <div className="rounded-xl border border-blue-200 dark:border-blue-800/60 bg-blue-50/70 dark:bg-blue-950/40 p-3 flex flex-col gap-2 shadow-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 text-xs text-slate-600 dark:text-slate-400 pt-1 border-t border-blue-100 dark:border-blue-900/40">
            <div className="flex items-center gap-1.5">
              <User className="w-3.5 h-3.5 text-slate-400 shrink-0" />
              <span>
                ผู้รับผิดชอบ: <strong className="text-slate-800 dark:text-slate-200">{activeItem.assigned_to ? (activeItem.assigned_to === operatorUid ? `ตนเอง (${operatorName})` : activeItem.assigned_to) : 'ยังไม่มอบหมาย'}</strong>
              </span>
            </div>
            {activeItem.next_action && (
              <div className="flex items-center gap-1.5 truncate">
                <span className="text-slate-400">ถัดไป:</span>
                <span className="truncate font-semibold text-slate-700 dark:text-slate-300">{activeItem.next_action}</span>
              </div>
            )}
            {activeItem.due_at && (
        ...
        <div className="flex flex-col gap-2">
          {latestCompleted && (
            <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 p-2.5 flex items-center justify-between text-xs text-slate-600 dark:text-slate-400">
              <div className="flex items-center gap-1.5">
                <History className="w-3.5 h-3.5 text-slate-400" />
                <span>
                  ประวัติงานล่าสุด: <strong>{WORK_ITEM_STATUS_LABELS[latestCompleted.status]}</strong> ({latestCompleted.resolution_summary || 'เสร็จสมบูรณ์'})
                </span>
              </div>
              <button
                type="button"
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white text-xs font-black rounded-xl transition-all shadow-sm hover:shadow cursor-pointer"
            >
